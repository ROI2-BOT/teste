...
const CONFIG = {
  ...
  // Mostra o aviso "isto é um link de exemplo" antes de ir para o
  // checkout enquanto CHECKOUT_URL ainda for o link de demonstração.
  // Assim que você trocar CHECKOUT_URL pelo link real, o aviso some
  // sozinho. Valores possíveis: "auto" | true | false.
  SHOW_EXAMPLE_LINK_WARNING: "auto",
...
function handleCheckoutClick(event) {
  ...
  const isPlaceholderLink = CONFIG.CHECKOUT_URL.includes("go.centerpag.com");
  const shouldWarn =
    CONFIG.SHOW_EXAMPLE_LINK_WARNING === true ||
    (CONFIG.SHOW_EXAMPLE_LINK_WARNING === "auto" && isPlaceholderLink);

  if (shouldWarn) {
    const proceed = confirm(
      "⚠️ ESTE ES UN LINK DE EJEMPLO. Busca \"checkout-exemplo.com\" en el archivo y reemplázalo por tu link real de checkout (Hotmart, Stripe, PayPal, etc.). ¿Deseas continuar de todos modos?"
    );
    if (!proceed) {
